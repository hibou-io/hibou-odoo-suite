...
def ee_401k(amount, rate, payslip, categories, worked_days, inputs):
    MAX = payslip.dict.rule_parameter('ee_401k_contribution_limit')
    if payslip.dict.ira_period_age() >= 50:
        MAX += payslip.dict.rule_parameter('ee_401k_catchup')
...
def er_401k_match(wages, payslip, categories, worked_days, inputs):
    MAX = payslip.dict.rule_parameter('er_401k_contribution_limit')
    ...
    rate = payslip.dict.contract_id.company_401k_match_percent(payslip)
