...
class Commission(models.Model):
    ...
    def name_get(self):
        ...
        for commission in self:
            ...
            if commission.invoice_id:
                name += commission.invoice_id.number
    ...
    @api.multi
    @api.onchange('invoice_id', 'contract_id', 'rate_type', 'base_amount', 'rate')
    def _compute_amount(self):
        for commission in self:
            ...
            rounding = 2
            if commission.invoice_id:
                rounding = commission.invoice_id.company_currency_id.rounding
                commission.base_total = commission.invoice_id.amount_total_company_signed
                commission.base_amount = commission.invoice_id.amount_for_commission()
    ...
    def _filter_invoices_for_creation(self, invoices):
        return invoices.filtered(lambda i: i.user_id and not i.commission_ids)

    @api.model
    def _commissions_to_confirm(self, invoices):
        commissions = invoices.mapped('commission_ids')
        return commissions.filtered(lambda c: c.state != 'cancel' and not c.move_id)

    @api.model
    def invoice_validated(self, invoices):
        employee_obj = self.env['hr.employee'].sudo()
        commission_obj = self.sudo()
        for inv in self._filter_invoices_for_creation(invoices):
            invoice_amount = inv.amount_for_commission()
            ...
            if commission_structure:
                commission_structure.create_for_invoice(inv, invoice_amount)
            else:
                employee = employee_obj.search([('user_id', '=', inv.user_id.id)], limit=1)
                contract = employee.contract_id
                if all((employee, contract)):
                    inv.commission_ids += commission_obj.create({
            ...
            if inv.commission_ids and inv.company_id.commission_type == 'on_invoice':
                commissions = self._commissions_to_confirm(inv)
                commissions.sudo().action_confirm()

        return True

    @api.model
    def invoice_paid(self, invoices):
        commissions = self._commissions_to_confirm(invoices)
        commissions.sudo().action_confirm()
        return True
    ...
    def action_confirm(self):
        ...
        for commission in self:
            ...
            date = commission.invoice_id.date_invoice if commission.invoice_id else fields.Date.context_today(commission)
            if commission.invoice_id.payment_move_line_ids:
                date = max(commission.invoice_id.payment_move_line_ids.mapped('date'))
...
class CommissionStructure(models.Model):
    ...
    def create_for_invoice(self, invoice, amount):
        self.ensure_one()
        ...
        for line in self.line_ids:
            ...
            if all((employee, rate)):
                ...
                if not line.rate:
                    ...
                    contract = employee.contract_id
                invoice.commission_ids += commission_obj.create({
                    'employee_id': employee.id,
