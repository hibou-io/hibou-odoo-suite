...
class SaleOrderMakePlan(models.TransientModel):
    ...
    def _next_warehouse_shipping_date(self, warehouse):
        hour_interval = warehouse.shipping_calendar_id.schedule_hours(0.01, compute_leaves=True)
        interval = warehouse.shipping_calendar_id.schedule_days(1, hour_interval[0][0], compute_leaves=True)
        return fields.Datetime.to_string(interval[0][1])
